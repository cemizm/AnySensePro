...
spiffs_file SPIFFS_open(spiffs *fs, const char *path, spiffs_flags flags, spiffs_mode mode)
{
	...
	if (res == SPIFFS_OK && (flags & (SPIFFS_CREAT | SPIFFS_EXCL)) == (SPIFFS_CREAT | SPIFFS_EXCL))
	{
...
s32_t SPIFFS_rename(spiffs *fs, const char *old, const char *new)
{
	...
	res = spiffs_object_update_index_hdr(fs, fd, fd->obj_id, fd->objix_hdr_pix, 0, (const u8_t*) new, 0, &pix_dummy);
